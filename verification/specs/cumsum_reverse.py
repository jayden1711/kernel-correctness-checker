"""KernelSpec for cumsum_reverse — f(x) -> reverse (suffix) sum over last dim

Added 2026-08-27 (Phase 1). Closed-form ||J_i||_2 lives in
verification/layer2_numeric_oracle/structural_l.py under op key "cumsum_reverse";
...
@dataclass
class CumsumReverseSpec(SingleTensorSpec):
    name: str = "cumsum_reverse"
...
def get_spec() -> CumsumReverseSpec:
    return CumsumReverseSpec(name="cumsum_reverse")
