"""KernelSpec for hardsigmoid — f(x) -> Tensor

Added 2026-08-27 (Phase 1). Closed-form ||J_i||_2 lives in
verification/layer2_numeric_oracle/structural_l.py under op key "hardsigmoid";
...
@dataclass
class HardsigmoidSpec(SingleTensorSpec):
    ...
    def get_adversarial_inputs(self, inputs):
        ...
        return [
            # The two kinks exactly. Off-by-one on the clamp bound is invisible elsewhere.
            ("at_breakpoints", _pack(torch.where(x > 0, torch.full_like(x, 3.0), torch.full_like(x, -3.0)))),
            # Pure linear region: (x+3)/6.
            ("inside_linear", _pack(x.clamp(-2.9, 2.9))),
            # Both saturated regions; must be exactly 0 or 1.
            ("outside_both", _pack(x.sign() * 1e3)),
        ]


def get_spec() -> HardsigmoidSpec:
    return HardsigmoidSpec(name="hardsigmoid")
