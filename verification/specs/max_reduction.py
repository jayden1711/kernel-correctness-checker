"""KernelSpec for max_reduction  f(x) -> Tensor(n_rows,), reduces last dim."""
...
class MaxReductionSpec(SingleTensorSpec):
    name: str = "max_reduction"
    ...
    def get_adversarial_inputs(self, inputs):
        x = inputs
        return [
            ("all_negative_nonpow2", -torch.rand(x.shape[0], 100, device=x.device) - 0.1),
            ("large_magnitude", x * 500),
        ]


def get_spec() -> MaxReductionSpec:
    return MaxReductionSpec(name="max_reduction")
