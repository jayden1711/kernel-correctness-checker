"""KernelSpec for new_gelu — f(x) -> Tensor, tanh-approximation GELU

Added 2026-08-27 (Phase 1). Closed-form ||J_i||_2 lives in
verification/layer2_numeric_oracle/structural_l.py under op key "new_gelu";
...
@dataclass
class NewGeluSpec(SingleTensorSpec):
    name: str = "new_gelu"
    ...
    def get_adversarial_inputs(self, inputs):
        ...
        return [
            # Output -> 0. The cubic term x^3 overflows in fp16 well before this.
            ...
            # The 0.044715*x^3 term dominates; catches a kernel that dropped it and shipped plain x*sigmoid(1.702x) instead.
            ("large_magnitude", _pack(x * 1e2)),
            # Must be ~x/2.
            ("near_zero", _pack(x * 1e-6)),
        ]


def get_spec() -> NewGeluSpec:
    return NewGeluSpec(name="new_gelu")
