"""KernelSpec for tanh — f(x) -> Tensor

Added 2026-08-27 (Phase 1). Closed-form ||J_i||_2 lives in
verification/layer2_numeric_oracle/structural_l.py under op key "tanh";
...
@dataclass
class TanhSpec(SingleTensorSpec):
    name: str = "tanh"
    ...
    def algebraic_properties(self):
        return [
            ('range_pm1', lambda cf, inputs: _check_range(cf, inputs, -1.0, 1.0)),
            ('odd_symmetry', lambda cf, inputs: _check_odd(cf, inputs)),
        ]
    ...
    def get_adversarial_inputs(self, inputs):
        ...
        return [
            ...
            # tanh(x) ~ x here; catches a wrong leading coefficient.
            ("near_zero", _pack(x * 1e-6)),
        ]


def get_spec() -> TanhSpec:
    return TanhSpec(name="tanh")
