"""KernelSpec for softplus — f(x, beta) -> Tensor

Added 2026-08-27 (Phase 1). Closed-form ||J_i||_2 lives in
verification/layer2_numeric_oracle/structural_l.py under op key "softplus";
...
@dataclass
class SoftplusSpec(KernelSpec):
    name: str = "softplus"
    requires_backward: bool = False

    # f(x, beta) -- beta is a fixed python float held constant
    ...
    def run_candidate(self, candidate_fn, inputs):
        x, beta = inputs
        return candidate_fn(x, beta)

    def run_reference(self, reference_fn, inputs):
        x, beta = inputs
        return reference_fn(x, beta)

    def primary_input(self, inputs):
        return inputs[0]

    def make_inputs(self, shape, device, dtype):
        """shape is (*tensor_shape, beta).

        The beta is VARIED, not fixed at the torch default. Fixing it was a
        ...
        """
        *tensor_shape, beta = shape
        return torch.randn(*tensor_shape, device=device, dtype=dtype), beta

    @property
    def algebraic_properties(self):
        return [
            ('strictly_positive', lambda cf, inputs: _check_positive(cf, inputs)),
        ]
    ...
    def get_adversarial_inputs(self, inputs):
        ...
        return [
            # log(1+exp(x)) OVERFLOWS without the linear-regime fallback -- softplus's
            # signature bug. 100.0, NOT 40.0: fp32 exp overflows near x=88, so at 40
            # (exp(40)=2.4e17, well inside range) the naive form is still FINITE and
            # the variant silently tested nothing. Measured -- the naive-overflow
            # mutant escaped at 40 and is caught at 100.
...
def get_spec() -> SoftplusSpec:
    return SoftplusSpec(name="softplus")
