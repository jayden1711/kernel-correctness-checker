...
@triton.jit
def causal_flash_attention_kernel(
    ...
    """Same tiling / online-softmax structure as flash_attention_kernel,
    with a correct causal mask added: query i attends to key j iff
    j <= i (standard GPT-style causal convention -- each position sees
    itself and everything before it, for MinGPTCausalAttention /
    MiniGPTBlock)."""
    ...
    for start in range(0, N, BLOCK_N):
        ...
        # Correct causal mask: query i attends to key j iff j <= i.
...
def causal_flash_attention(Q, K, V):
    ...
    causal_flash_attention_kernel[grid](
        Q, K, V, O,
