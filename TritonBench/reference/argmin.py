...
@triton.jit
def argmin_kernel(output_ptr, input_ptr, input_row_stride,
    ...
    # First-occurrence tie-break, same convention as argmax.
    is_min = (row == min_val) & mask
    candidate_idx = tl.where(is_min, col_offsets, n_cols)
    argmin_idx = tl.min(candidate_idx, axis=0).to(tl.int64)
