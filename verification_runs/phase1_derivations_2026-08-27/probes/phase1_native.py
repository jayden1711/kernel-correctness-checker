...
OUT = "/content/phase1_native.jsonl"
...
for oi, op in enumerate(OPS):
    ...
    for inv in range(N_INV):
        ...
        # valid_shapes[0] for EVERY invocation, varying only the random draw.
        #
        ...
        # this mirrors that.
        inputs = spec.make_inputs(spec.valid_shapes[0], "cuda", torch.float32)
        x, rest = split(inputs)
        try:
            r = measure(KERNELS[op], x, rest, seed=1000 + 7 * oi + inv)
