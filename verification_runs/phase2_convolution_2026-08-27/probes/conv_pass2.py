...
OUT = "/content/conv_pass2.jsonl"
NS, ETA, DELTA_SCALE = 40, 0.05, 1e-3
K_LADDER = [400, 4000, 20000]
K_SUB = ["conv1d","conv2d","conv3d","conv_transpose1d","conv_transpose2d","conv_transpose3d","depthwise_conv2d","pointwise_conv2d"]
...
for op in K_SUB:
    ...
    x, rest = split(inp)
    fn = (lambda t, *_i, _s=spec, _k=KERNELS[op], _in=inp:
          _s.run_candidate(_k, (t,) + tuple(_in[1:])))
    base = fn(x, *rest)
    xs = x.float().std().item()
